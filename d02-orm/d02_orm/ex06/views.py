...
def update(request):
    try:
        ...
        if request.method == "POST":
            id_to_update = request.POST.get('movies')
            new_opening_crawl = request.POST.get('opening_crawl')
            if id_to_update and new_opening_crawl:
                cursor.execute("""
                    UPDATE ex06_movies
                    SET opening_crawl = %s
                    WHERE episode_nb = %s;
                """, (new_opening_crawl, int(id_to_update)))
                conn.commit()
            cursor.close()
            conn.close()
            return redirect('update')

        cursor.execute("SELECT episode_nb, title, director FROM ex06_movies;")
        movies = cursor.fetchall()
        cursor.close()
        conn.close()
